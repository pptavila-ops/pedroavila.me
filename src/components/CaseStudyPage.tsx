...
import { CommandFlowDiagram } from './CommandFlowDiagram';
...
type Translate = <T extends string | undefined>(text: T) => T;

function renderSection(section: RichSection, i: number, t: Translate) {
    switch (section.type) {
        case 'text':
            return section.html ? (
        ...
        case 'process':
            return (
                <div key={i} className="mt-16 flex flex-col gap-0">
                    {section.title && (
                        <p className="text-sm font-semibold uppercase tracking-widest text-white/50 mb-8">{t(section.title)}</p>
                    )}
                    {section.steps.map((step, j) => (
                        <div key={step.title} className="flex gap-5 group">
                            ...
                            <div className={j < section.steps.length - 1 ? 'pb-8' : ''}>
                                ...
                                <p className="text-[15px] font-normal text-white/55 leading-relaxed mt-2">{t(step.description)}</p>
                            </div>
                        </div>
                    ))}
                </div>
        ...
        case 'image':
            return (
                <figure key={i} className={`${section.compact ? 'mt-2' : 'mt-8'} ${section.tight ? '-mb-10' : ''} ${section.shrink ? 'md:w-4/5 md:mx-auto' : ''}`}>
                    <FadeImage
                        src={section.src}
                        alt={t(section.caption) || ''}
                            ...
                            <div key={bar.label} className="flex flex-col gap-3">
                                <div className="flex items-baseline justify-between gap-4">
                                    <div className="flex items-baseline gap-2.5">
                                        <span className="text-[26px] md:text-[30px] font-bold text-white leading-none tracking-tight">{t(bar.label)}</span>
                                    </div>
                                </div>
                                ...
                                <span className="text-[17px] md:text-[18px] text-white/55">{t(bar.description)}</span>
                            </div>
                        ))}
                    </div>
                    {section.caption && (
                        <p className="mt-5 text-[13px] text-white/50 text-center">{t(section.caption)}</p>
                    )}
                </div>
            );
        ...
        case 'animated-sentence':
            return <AnimatedSentence key={i} prefix={t(section.prefix)} phrases={section.phrases.map((phrase) => t(phrase))} />;

        case 'command-flow-diagram':
            return <CommandFlowDiagram key={i} />;
    ...
    return (
        <div>
            ...
            {study.brief ? (
                ...
                <p className="mt-8 pt-8 border-t border-white/10 text-lg md:text-xl font-normal leading-relaxed text-white/70" dangerouslySetInnerHTML={{ __html: t(study.intro) }} />
            ) : (
            ...
            {/* Sections */}
            {study.sections.map((section, i) => renderSection(section, i, t))}
