...
function App() {
    ...
    useEffect(() => {
        let title: string = t('Pedro Ávila — Senior Product Designer');
        ...
        if (activePlayground) {
            ...
            if (current) title = `${t(current.title)} — Pedro Ávila`;
        }

        document.title = title;
    }, [activeStudy, activePlayground, t]);
        ...
        <main className="flex min-h-screen bg-black text-white">
            ...
            <div className="hidden md:flex md:w-[32%] lg:w-[28%] fixed top-0 left-0 h-screen flex-col justify-between px-8 lg:px-10 py-10">
                <div>
                    ...
                    {/* mt-6 matches the gap before the skill tags below, so the
                    ...
                    <div className="flex flex-wrap gap-2 mt-6">
                        {[
                            { label: 'Design to Code', tip: 'I close the gap between design and front-end engineering: turning design decisions into production-ready components and features, and enabling designers across the org to work directly in code instead of static tools.' },
                            { label: 'Claude Code', tip: 'I build interfaces and prototypes with Claude as a coding partner, and own the UX space for AI skills at HelloFresh, writing new skills and improving existing ones.' },
                            ...
                            { label: 'Design Ops', tip: 'I make design system components, prototyping workflows, and code delivery tools available to every designer in the org through guides, 101 sessions, and a culture that fosters curiosity instead of overwhelm.' },
                        ].map((tag) => (
                            <Tooltip key={`${tag.label}-${shineKey}`} text={t(tag.tip)}>
                                <span className="text-[13px] text-white/60 hover:text-white rounded-md px-3 py-1 cursor-default animate-tag-shine hover:border-white/40 transition-colors">{t(tag.label)}</span>
                            </Tooltip>
                        ))}
                        <Tooltip
                            key={`music-${shineKey}`}
                            text={t('I\'ve also been learning music production and experimenting with sounds and synthesizers I enjoy.')}
                ...
                <div className="mt-6">
                    <div className="flex items-center gap-4">
                        ...
                        <a href="https://github.com/pptavila-ops" target="_blank" rel="noreferrer" className="text-white/50 hover:text-white transition-colors">
                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" clipRule="evenodd" d="M12 0C5.37 0 0 5.24365 0 11.7176C0 16.9027 3.435 21.2822 8.205 22.8348C8.805 22.9373 9.03 22.5858 9.03 22.2782C9.03 21.9999 9.015 21.0771 9.015 20.0958C6 20.6377 5.22 19.3781 4.98 18.7189C4.845 18.3821 4.26 17.3421 3.75 17.0638C3.33 16.8441 2.73 16.3022 3.735 16.2875C4.68 16.2729 5.355 17.1371 5.58 17.4886C6.66 19.2609 8.385 18.7629 9.075 18.4553C9.18 17.6936 9.495 17.181 9.84 16.8881C7.17 16.5951 4.38 15.5845 4.38 11.1025C4.38 9.82817 4.845 8.77359 5.61 7.95335C5.49 7.66041 5.07 6.45935 5.73 4.84818C5.73 4.84818 6.735 4.54059 9.03 6.04923C9.99 5.78559 11.01 5.65376 12.03 5.65376C13.05 5.65376 14.07 5.78559 15.03 6.04923C17.325 4.52594 18.33 4.84818 18.33 4.84818C18.99 6.45935 18.57 7.66041 18.45 7.95335C19.215 8.77359 19.68 9.81353 19.68 11.1025C19.68 15.5991 16.875 16.5951 14.205 16.8881C14.64 17.2542 15.015 17.9573 15.015 19.0558C15.015 20.6231 15 21.8827 15 22.2782C15 22.5858 15.225 22.9519 15.825 22.8348C20.565 21.2822 24 16.8881 24 11.7176C24 5.24365 18.63 0 12 0Z" fill="currentColor" /></svg>
                        </a>
                        <div className="w-px h-4 bg-white/20" />
                        <Tooltip text={t('Click to copy')}>
                            <button onClick={handleCopyEmail} className="grid text-sm font-semibold text-white/50 hover:text-white transition-colors cursor-pointer">
                                <span className={`col-start-1 row-start-1 inline-flex items-center gap-2.5 ${copied ? 'invisible' : ''}`}>
                                ...
                                <span className={`col-start-1 row-start-1 inline-flex items-center gap-2.5 ${copied ? '' : 'invisible'}`}>
                                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M18.0195 7.05028C18.41 6.65973 19.0439 6.65978 19.4345 7.05028C19.8247 7.44073 19.8247 8.07388 19.4345 8.46434L10.9501 16.9497C10.7626 17.1372 10.5073 17.2427 10.2421 17.2427C9.97706 17.2425 9.72251 17.1371 9.53508 16.9497L5.29289 12.7075C4.90237 12.317 4.90237 11.683 5.29289 11.2925C5.68336 10.9023 6.31649 10.9023 6.70696 11.2925L10.2421 14.8276L18.0195 7.05028Z" fill="currentColor" /></svg>
                                    {t('Copied')}
                                </span>
                            </button>
                        </Tooltip>
                        <div className="w-px h-4 bg-white/20" />
                        <a href="/cv.pdf" download className="inline-flex items-center gap-2.5 text-sm font-semibold text-white/50 hover:text-white transition-colors">
            ...
            <div className="w-full min-w-0 md:ml-[32%] lg:ml-[28%] min-h-screen px-7 py-10 md:py-14 md:px-10 lg:px-14 overflow-x-clip">
                ...
                <div className={`md:hidden mb-10 pb-8 border-b border-white/10 ${activeStudy ? 'hidden' : ''}`}>
                    ...
                    <p className="text-[16px] text-white/60 mt-1 leading-relaxed">{t('Senior Product Designer working across AI workflows, design tooling, and production code.')}<br />{t('He, him')}</p>
                ...
                {activePlayground ? (
                    ...
                    <div>
                        ...
                        <button
                            onClick={closeStudy}
                            className="inline-flex items-center gap-2.5 text-sm text-white/50 hover:text-white transition-colors cursor-pointer"
                        >
